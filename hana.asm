...
PROC EXCHANGE_NAMES
    ...
    ; ////////////////////////////// get player2 name
    ; send my name to the other player using serial port
    ; receive the other player name from the serial port
    ...
    ;Check that Transmitter Holding Register is Empty
            mov dx , 3FDH		; Line Status Register
    CHK_TRANSIMITER_NAME:  	In al , dx 			;Read Line Status
            AND al , 00100000b
            JZ CHK_TRANSIMITER_NAME
...
PROC getMode
        ...
        ;if there is something in the buffer jmp to recevier mode
        ;else jmp to sender mode

        ;receiver mode:
        ;send my name to the other player using serial port
        ;receive the other player name from the serial port
        ;display the notification message of the corrresponding invitaion
        ;wait for the other player to corresponding key to accept the invitaion
        ;if the other player pressed f1 --> Enter Game
        ;if the other player pressed f2 --> Enter Chat
        ;if the other player pressed f3 --> Exit Game

        ;sender mode:
        ; wait for key to send the invitaion
        ;send the other player name to the other player using serial port
        
        MOV AH,01H
        INT 21H
        CMP AL,0
        JNE WaitGame
        ;check if the user pressed f1 or f2 or f3
...
playerName1              DB           17 dup(' '),"$"
playerName2             DB           17 dup(' '),"$"
scoreLabel1       DB "  000","$"
scoreLabel2       DB "  000","$"
