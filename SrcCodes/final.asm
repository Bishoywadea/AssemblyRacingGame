...
;Date : 28-11-2023
EXTRN Inlinechat:far
public char, isout, chaty, chatx,recieveflag,counter
IDEAL
MODEL compact
STACK 100h
...
PROC printScore 
;;stringify score1 and score 2,decide x and y and call print string at loc 
push ax
push dx
push bx
push cx
...
PROC CheckScore  ;;size of coordinates array=num of sections moved*5 words

push ax
push si
push cx

;;loop on all array retrieve x1,x2,y1,y2
;;check if player1x and player1 y are within boundaries
;;if within bounds update score
lea si,track_coords
mov cl,1 ;;section index msln

players_score:
push cx
;;;;;;;;;;;;;;;;retrieve section's x1,x2,y1,y2
mov ax,[si]
mov x1,ax
add si,2
mov ax,[si]
...
mov ax,x1
cmp XPlayer1,ax
jl skip_player1_check ;;if player1 x<x1 ,,not within bounds
mov ax,x2
cmp XPlayer1,ax  ;;if player1x>x1 and >x2, not within bounds
jg skip_player1_check
mov ax,y1
cmp YPlayer1,ax
jl skip_player1_check
;;;;;;;;;;;;;;;;;;;;intermediate jmp
jmp skip_score_inter
players_score_inter:
jmp players_score
skip_score_inter:
;;;;;;;;;;;;;;;;;;;;
...
mov bl,100
mul bl  
div numsectionsmoved
mov score1,al

skip_player1_check:
mov ax,x1
cmp XPlayer2,ax
jl skip_player2_check
mov ax,x2
cmp XPlayer2,ax
jg skip_player2_check
mov ax,y1
cmp YPlayer2,ax
jl skip_player2_check
mov ax,y2
...
pop cx
inc cl
cmp cl,numsectionsmoved
jnz players_score_inter
...
PROC drawleft
                        ...
                        CMP  row, downlimit-trackwidth
                        jg   skip02 ;;terminate left inter
                        CMP  col,leftlimit+trackwidth
                        jle  skip02 ;;terminate left inter
                        CMP  lastmove,0
                        jnz  skip4
                        add  col,trackwidth
...
PROC PrintWinner
    ...
    printStringAtLoc scoreLabel1 12 61
    printStringAtLoc playerName2 14 46
    printStringAtLoc scoreLabel2 14 61
    ...
    printStringAtLoc scoreLabel2 12 71
    printStringAtLoc playerName1 14 46
    printStringAtLoc scoreLabel1 14 71
ret
ENDP
...
PROC main
 ...
 ;;; get the Address of the existing int09h handler to modify it and enable multiple movements at the same time
    ...
    mainLoop:
        ...
        printStringAtLoc scoreLabel1 29 57
        printStringAtLoc scoreLabel2 29 17
        CMP [reDrawFlag],1
        JNE skipReDraw
        CALL reDrawObs
     skipReDraw:
        ...
        SKIP_GenerationObstacles:
            ...
            mov dx , 3F8H		; Transmit data register
            mov al,[sendData]       ; put the data into al
            out dx , al         ; sending the data
...
track_coords dw 200 dup(?)
    ...
    score1 db 0  
    score2 db 0
    scoreLabel1 db "  score:000",'$'
    scoreLabel2 db "  score:000",'$'
    x1 dw ?
    x2 dw ?
    y1 dw ?
    y2 dw ?
    ...
    PowerUpsPlayer1_XLoc EQU 313D
    PowerUpsPlayer2_XLoc EQU 628D
